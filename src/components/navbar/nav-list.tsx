...
export function NavigationList({ navigation }: Props) {
  ...
  const renderRootLevel = (list: Menu[]) => {
    return list.map((nav) => {
      ...
      if (nav.child && nav.child.length > 0 && nav.megaMenu === false && nav.megaMenuWithSub === false) {
        return (
          <FlexBox
            key={nav.title}
            alignItems="center"
      ...
      // SHOW SIMPLE LINK WITHOUT DROPDOWN (no children)
      if ((!nav.child || nav.child.length === 0) && nav.megaMenu === false && nav.megaMenuWithSub === false) {
        const url = nav.url || "/";
        return (
          <NavLink href={url} key={nav.title}>
