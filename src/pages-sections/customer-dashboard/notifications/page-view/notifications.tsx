...
import Notifications from "icons/MobileNotch";
...
export function NotificationsPageView() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      setError(err.response?.data?.message || "خطا در دریافت اعلانها");
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  if (loading) {
    return (
      <Fragment>
        <DashboardHeader Icon={Notifications} title="اعلانها" />
    ...
    return (
      <Fragment>
        ...
        <Box py={8} textAlign="center">
          <Notifications sx={{ fontSize: 80, color: "grey.300", mb: 2 }} />
