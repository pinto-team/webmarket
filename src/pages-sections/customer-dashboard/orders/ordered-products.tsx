  ...
  return (
    <Card
      ...
      <FlexBetween px={3} py={2} flexWrap="wrap" gap={1} bgcolor="grey.50">
        <Item title="کد سفارش:" value={order.code} />
        <Item title="تاریخ ثبت:" value={format(new Date(order.created_at), "yyyy/MM/dd")} />
        <Item title="وضعیت:" value={order.status_label} />
      </FlexBetween>

      {order.shipments.map((shipment) =>
        shipment.items.map((item) => (
          <FlexBetween px={2} py={1} flexWrap="wrap" key={item.id} gap={1}>
            <FlexBox gap={2} alignItems="center">
              <Avatar variant="rounded" sx={{ height: 60, width: 60, backgroundColor: "grey.50" }}>
                <img
                  alt={item.sku_data.title}
                  src={getProductImageUrl(item.sku, "60x60")}
