...
export default function PaymentPageViewNew() {
  ...
  useEffect(() => {
    if (isExpired) {
      enqueueSnackbar('زمان پرداخت به پایان رسید', { variant: 'error' });
      router.push('/cart');
    }
  }, [isExpired]);
  ...
  useEffect(() => {
    ...
    const loadOrder = async () => {
      try {
        ...
        enqueueSnackbar('خطا در بارگذاری سفارش', { variant: 'error' });
        router.push('/checkout');
      } finally {
  ...
  const handleProceedToPayment = async () => {
    if (!selectedGatewayId && !selectedShopGatewayId) {
      enqueueSnackbar('لطفا یک درگاه پرداخت انتخاب کنید', { variant: 'warning' });
      return;
    }

    if (order?.shipments && order.shipments.length > 0 && cargoSelections.length !== order.shipments.length) {
      enqueueSnackbar('لطفا روش ارسال را برای تمام مرسولهها انتخاب کنید', { variant: 'warning' });
      return;
    }
    ...
    try {
      ...
      if (response?.payment_url) {
        ...
        enqueueSnackbar('خطا در دریافت لینک پرداخت', { variant: 'error' });
      }
    } catch (err: any) {
      enqueueSnackbar(err.response?.data?.message || 'خطا در ایجاد پرداخت', { variant: 'error' });
    }
  };
  ...
  return (
    <>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        <Typography variant="h4">پرداخت</Typography>
        ...
        <Grid size={{ md: 8, xs: 12 }}>
          ...
          <Card
            ...
            <GatewaySelector
              ...
              loading={false}
              error={!gatewayData?.gateways?.length ? 'درگاه پرداختی موجود نیست' : null}
            />
          </Card>
        </Grid>
