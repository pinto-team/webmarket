...
export default function CheckoutSummary() {
  ...
  const { subtotal, tax, total, discount, shipping } = useMemo(() => {
    ...
    const subtotal = cart.reduce((sum, item) => sum + item.sku.price * item.quantity, 0);
    const tax = 40;
    const shipping = 0;
    const discount = 0;
      ...
      {cart.map((item) => (
        <FlexBetween mb={1.5} key={item.id}>
          <Typography variant="body1">
            <strong>{item.quantity}</strong> x {item.sku.title}
          </Typography>

          <Typography variant="body1">{currency(item.sku.price)}</Typography>
        </FlexBetween>
      ))}
