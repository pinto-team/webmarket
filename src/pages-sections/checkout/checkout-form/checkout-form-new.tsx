...
import { OrderCreateRequest, CargoMethodSelection, OrderResource } from "@/types/order.types";
import { OrderTimer, OrderDraftSummary, PaymentFinalization } from "@/components/order";
...
export default function CheckoutFormNew() {
  ...
  const { user } = useAuth();
  const { createOrder, loading: creating, error: createError, orderId } = useOrderCreate();
  ...
  const handleCargoChange = (shipmentId: number, methodId: number) => {
    setCargoSelections(prev => {
      ...
      if (existing) {
        return prev.map(s => s.shipment_id === shipmentId ? { shipment_id: shipmentId, shop_cargo_method_id: methodId } : s);
      }
  ...
  if (step === "create") {
    return (
      <CardRoot elevation={0}>
        <Typography variant="h5" mb={2}>Customer Information</Typography>
