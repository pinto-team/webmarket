...
export default function CheckoutPageView() {
  ...
  const handleProceedToPayment = async () => {
    if (!selectedAddressId) {
      enqueueSnackbar("لطفا آدرس تحویل را انتخاب کنید", { variant: "warning" });
      setIsModalOpen(true);
      return;
    }

    if (cart.length === 0) {
      enqueueSnackbar("سبد خرید خالی است", { variant: "warning" });
      return;
    }
    ...
    try {
      ...
      if (order.gateways && order.gateways.length > 0) {
        localStorage.setItem('checkout_gateways', JSON.stringify({
      ...
      // Error already shown by interceptor, just stop loading
      setLoading(false);
    }
  };
