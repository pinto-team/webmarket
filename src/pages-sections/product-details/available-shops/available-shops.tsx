...
export default function AvailableShops({ skus, productCode }: Props) {
    ...
    const isInCart = (skuId: number) => cart.some((item) => item.sku?.id === skuId);

    return (
        <Box sx={{ mb: 4 }} id="sellers-section">
            <Typography variant="h3" sx={{ mb: 3, mt: 4 }}>
                {t("productDetail.allSellers")}
            </Typography>
            ...
            {shopGroups.flatMap((group) =>
                group.skus.map((sku) => {
                    ...
                    const guaranteeText = sku.guarantee
                        ? `${toPersianNumber(sku.guarantee.duration)} ${t("productDetail.monthWarranty")}`
                        : t("productDetail.warrantyAvailable");
                        ...
                        <StyledCard key={`${group.shop.title}-${sku.id}`}>
                            ...
                            <Box className="delivery-info">
                                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                                    ...
                                    <Typography variant="body2">{guaranteeText}</Typography>
                                </Box>

                                <Typography variant="body2" color={hasStock ? "success.main" : "error.main"}>
                                    {hasStock ? t("productDetail.available") : t("productDetail.outOfStock")}
                                </Typography>
                            </Box>
