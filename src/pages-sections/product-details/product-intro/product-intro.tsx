  ...
  return (
    <StyledRoot>
      <Grid container spacing={3}>
        ...
        <Grid size={{ lg: 6, md: 6, xs: 12 }}>
          <Typography variant="h1" sx={{ mb: 2.5 }}>{product.title}</Typography>
          ...
          {product.categories[0] && (
            <Typography variant="body1" sx={{ mb: 1.5 }}>
              ...
              <Link href={`/category/${product.categories[0].slug}`} style={{ textDecoration: 'none' }}>
                <strong style={{ color: 'inherit', cursor: 'pointer', textDecoration: 'underline' }}>{product.categories[0].name}</strong>
              </Link>
            </Typography>
          )}
          ...
          {product.brands[0] && (
            <Typography variant="body1" sx={{ mb: 1.5 }}>
              ...
              <Link href={`/brand/${product.brands[0].code}`} style={{ textDecoration: 'none' }}>
                <strong style={{ color: 'inherit', cursor: 'pointer', textDecoration: 'underline' }}>{product.brands[0].title}</strong>
              </Link>
            </Typography>
          )}
          ...
          <Typography variant="body1" sx={{ mb: 2.5 }}>
            {t("Product Code")}: <strong>{product.code}</strong>
          </Typography>
          ...
          <div className="rating" style={{ marginBottom: '20px' }}>
            ...
            <Typography variant="h6">({toPersianNumber(product.comments?.length || 0)})</Typography>
          </div>
          ...
          {firstShop && (
            <Card sx={{ p: 2, mb: 2, backgroundColor: '#f9f9f9' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                ...
                <Typography variant="subtitle1" fontWeight={600}>{firstShop.shop.title}</Typography>
              </Box>
              {hasMultipleShops && (
                <Button 
                  size="small" 
                ...
                >
                  {t("View")} {toPersianNumber(shopGroups.length - 1)} {t("Other Sellers")}
                </Button>
              )}
            </Card>
          )}
