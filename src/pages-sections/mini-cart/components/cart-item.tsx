...
export default function MiniCartItem({ item, onCart }: Props) {
    ...
    const total = (item.sku?.price || 0) * (item.quantity || 0);
    const currencyLabel = t("products.currencyLabel");

    return (
        <StyledRoot>
            <Link href={productCode ? `/products/${productCode}` : "#"}>
                <StyledAvatar variant="rounded">
                    <ProductImage
                        product={item.sku?.product || item.sku}
                        alt={item.sku?.title || "product"}
                        width={80}
                        height={80}
                        size="80x80"
                ...
                <FlexBox alignItems="center" justifyContent="space-between" gap={1} flexWrap="nowrap">
                    <QuantityWrapper>
                        <Button
                            ...
                            color="primary"
                            variant="text"
                            onClick={onCart((item.quantity || 0) + 1, item)}
                        >
                            <Add fontSize="small" />
                        </Button>

                        <Typography variant="body2" fontSize={13} sx={{ minWidth: 20, textAlign: "center" }}>
                            {toPersianNumber(item.quantity || 0)}
                        </Typography>
