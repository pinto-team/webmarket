...
type ImageLikeEntity = {
  ...
  image?: { proxy_url?: string };
  icon?: { proxy_url?: string };
};
...
export const getServerImageUrl = (
  ...
  const normalizedEntity = entity as
    | ImageLikeEntity
    | string
    | null
    | undefined;

  // Accept direct proxy_url string too.
  const raw =
    typeof normalizedEntity === "string"
      ? normalizedEntity
      : normalizedEntity?.upload?.proxy_url ||
        normalizedEntity?.proxy_url ||
        normalizedEntity?.image?.proxy_url ||
        normalizedEntity?.icon?.proxy_url ||
        "";
