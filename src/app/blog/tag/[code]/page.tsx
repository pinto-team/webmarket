...
export default function BlogTagPage() {
  ...
  const { data, loading, error } = usePostTag(code, queryParams);
  ...
  if (error || !data) {
    return (
      <Container sx={{ py: 4 }}>
        <Typography variant="h4">برچسب یافت نشد</Typography>
      </Container>
    );
  }
  ...
  return (
    <Container sx={{ py: 4 }}>
      <Breadcrumbs sx={{ mb: 3 }}>
        ...
        <Typography color="text.primary">{data.tag.name}</Typography>
      </Breadcrumbs>

      <Typography variant="h3" sx={{ mb: 4, fontWeight: 700 }}>برچسب: {data.tag.name}</Typography>
